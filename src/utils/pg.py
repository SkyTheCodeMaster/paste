...
class PGUtils:
  ...
  async def verify_token(self, token: Union[Token,str]) -> Union[Token,bool]:
    ...
    async with self.pool.acquire() as conn:
      ...
      if record:
        ...
        record = await conn.fetchrow("SELECT * FROM Users WHERE Password = $1",token)
        if record:
          user = User(
  ...
  async def _generate_token_id(self) -> str:
    ...
    async with self.pool.acquire() as conn:
      record = await conn.fetchrow("SELECT EXISTS (SELECT 1 FROM APITokens WHERE Id = $1);",pasteID)
      if record["exists"]:
  ...
  async def create_new_paste(self, paste: Paste, token: Union[Token,str]) -> Union[Paste,bool]:
    ...
    async with self.pool.acquire() as conn:
      await conn.execute("INSERT INTO Pastes VALUES ($1, $2, $3, $4, $5);",pasteID,token.owner.id,paste.data,paste.visibility,paste.title)
  ...
  async def edit_paste(self, paste:Paste, token: Union[Token,str]) -> Union[Paste,bool]:
    ...
    if token.owner.id != nPaste.creator:
      return False
    async with self.pool.acquire() as conn:
      await conn.execute("UPDATE Pastes SET Content = $1, Title = $2, Visibility = $3 WHERE id = $4",paste.data,paste.title,paste.visibility,nPaste.id)
